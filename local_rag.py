"""
Local RAG — ChromaDB vector store + SentenceTransformers + optional BM25 hybrid search.
"""
...
def _unwrap_chroma(data: Any) -> list:
    """
    Chroma wraps single-query results in an extra list: ``[[doc1, doc2]]``.
    Unwrap when present so callers always get a flat list.
    """
    if not data:
        return []
...
def _build_where_filter(
    ...
    """Build a Chroma ``where`` clause, or ``None`` if no filters apply."""
...
class MergedLocalRAG:
    ...
    def _chunk_id(file_info: Dict[str, str], chunk: Dict[str, Any]) -> str:
        """Deterministic, filesystem-safe chunk identifier."""
    ...
    def _is_already_ingested(self, file_info: Dict[str, str]) -> bool:
        """
        Check whether *any* chunk from this file exists in the collection.

        Uses a metadata query — much more reliable than guessing a single
        chunk id (which silently skips partially-ingested files).
        """
        try:
            ...
            # If the query fails (e.g. empty collection), assume not ingested
            return False

    def ingest_file(
        self, file_info: Dict[str, str], rebuild_bm25: bool = True
    ) -> int:
        """
        Ingest a single file into the collection.

        Returns:
            Number of chunks added (0 if skipped or failed).
        """
        if self._is_already_ingested(file_info):
            logger.info("⏭️ Already ingested: %s", file_info["file_name"])
            return 0
        ...
        try:
            ...
            if not chunks:
                logger.warning("⚠️ No text extracted from %s", file_path)
                return 0

            ids, documents, metadatas = self._prepare_batch(
                file_info, chunks, file_path
            )
    ...
    def _extract_chunks(
        ...
        """Route to the correct extractor, then chunk the result."""
        file_path = file_info["full_path"]

        if ext == ".pdf":
            return self.pdf_processor.process_pdf(file_path)

        # Non-PDF: use document_processor → then semantic_chunking
    ...
    def _prepare_batch(
        ...
        """Build parallel id / document / metadata lists for Chroma .add()."""
        ids: List[str] = []
        documents: List[str] = []
    ...
    def ingest_directory(
        ...
        """Ingest every supported file found under *data_dir*."""
        if data_dir is None:
    ...
    def _rebuild_bm25_index(self) -> None:
        """(Re)build the BM25 index from the full Chroma collection."""
        try:
    ...
    def search(
        ...
        Delegates to hybrid search when BM25 is enabled, otherwise
        pure semantic.  Return schema is **identical** in both modes.
        """
        n_results = n_results or get_config().default_search_results

        if self.enable_bm25:
            return self.hybrid_search(
                query, n_results, subject_filter, module_filter
            )
    ...
    def _semantic_search(
        ...
        """Pure vector search (no BM25 component)."""
        try:
    ...
    def hybrid_search(
        ...
        Returns the top *n_results* ranked by weighted hybrid score.
        """
        config = get_config()
        semantic_weight = (
        ...
        try:
            ...
            # 2. BM25 candidates (merged in-place)
            if self.enable_bm25:
                self._merge_bm25_candidates(
                    combined, query, n_results, subject_filter, module_filter
                )
    ...
    def _semantic_candidates(
        ...
        """Run a semantic query and return ``{doc_id: entry}``."""
        emb = self._embed_texts([query])
        where = _build_where_filter(subject_filter, module_filter)
    ...
    def _merge_bm25_candidates(
        ...
        """Score with BM25 and merge into *combined* in-place."""
    ...
    def _doc_id(md: Dict[str, Any]) -> str:
        """Consistent document identifier from metadata."""
        return (
    ...
    def _distances_to_scores(distances: List[float]) -> List[float]:
        """
        Convert Chroma L2 distances → 0–1 similarity scores.

        Uses min–max normalisation **within the result batch**, so scores
        are relative rankings, not absolute similarity measures.
        """
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        """Chunk count, distinct subjects, distinct modules."""
        try:
    ...
    def get_organization_info(self) -> Dict[str, Any]:
        """Database stats + on-disk file structure."""
        stats = self.get_collection_stats()
        try:
            structure = get_organization_structure(str(get_config().data_dir))
        except Exception:
            logger.debug("Could not fetch organization structure", exc_info=True)
            structure = {}
        return {"database_stats": stats, "file_structure": structure}

    def clear_database(self) -> bool:
        """Drop and recreate the collection.  Returns ``True`` on success."""
        try:
